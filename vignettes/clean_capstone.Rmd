...
new_m <- as.data.frame(lapply(new_m, function(x){
  x <- replace(x, x %in% c("NA", "N/A", ".", "na","-", "8__unknown", "dont_know", 
                           "5__unknown","Do_not_know", "No Response", "no responde",
                           "No sabe", "Don't know","no sabe", "no_responde", 
                           "Not specified.", "", "no answer", "No answer", 
                           "sin respuesta", "Sin respuesta", "Sin Informacion", 
                           "Unknown", "UNKNOWN", "unknown", "Unkown", "unkown", 
                           "Not Specified", "Desconocido","desconocido", 
                           "6__unknown", "7__no_answer", "NULL", 
                           "Sin respuesta, Por que?", "NO ANSWER"), NA)
  }))
...
```{r}
new_m[,1:ncol(new_m)] <- lapply(new_m[,1:ncol(new_m)], sub, 
                                pattern = "^F.*(emale|emenino)$", replacement = "Female",
                                ignore.case=TRUE)
new_m[,1:ncol(new_m)] <- lapply(new_m[,1:ncol(new_m)], sub, 
                                pattern = "^M.*(ale|asculino)$", replacement = "Male",
                                ignore.case=TRUE)
